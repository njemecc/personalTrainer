...
} from "@/lib/actions/nutritionPlan.actions";
...
const DodajPlanIshrane = ({
  isOpen,
  onClose,
  userId,
  onPlanAdded,
}: DodajPlanIshraneProps) => {
  ...
  const handleAssignPlan = async () => {
    ...
    if (!userId) {
      alert(
        "Greška: ID korisnika nedostaje. Molimo osvežite stranicu i pokušajte ponovo."
      );
    ...
    try {
      console.log(
        "Attempting to assign plan. UserId:",
        userId,
        "PlanId:",
        selectedPlan
      );
      ...
      let errorMessage =
        "Došlo je do greške prilikom dodavanja plana ishrane. Molimo pokušajte ponovo.";

      if (error instanceof Error && error.message.includes("database")) {
        errorMessage =
          "Problem sa vezom ka bazi podataka. Molimo pokušajte ponovo za nekoliko trenutaka.";
      }
                ...
                {isLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center p-10 bg-amber-50 dark:bg-gray-800 rounded-lg border border-amber-200 dark:border-amber-900/30">
                    <p className="text-gray-600 dark:text-gray-400 text-center">
                      {searchTerm
                        ? "Nema planova ishrane koji odgovaraju pretrazi."
                ...
                <div className="bg-amber-50 dark:bg-gray-800 p-4 rounded-lg border border-amber-200 dark:border-amber-900/30 w-full">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300">
                      Nakon uploada fajla, plan će biti dostupan u sekciji
                      &quot;Postojeći Planovi&quot;
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300">
                      Podržani formati:{" "}
                      <strong className="text-red-500">PDF</strong>,{" "}

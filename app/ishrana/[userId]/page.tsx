import { getAllNutritionPlansByUserId } from "@/lib/actions/nutritionPlan.actions";
...
const NutritionPlanPage = async ({ params }: UserDetailsPageParams) => {
  ...
  if (!user.is_active) {
    return (
      <div className="flex flex-col items-center justify-center gap-6 py-20">
        <h1 className="text-xl md:text-3xl font-semibold text-center">
          ❌ Pristup planu ishrane je trenutno onemogućen.
        </h1>
  ...
  const nutritionPlans = await getAllNutritionPlansByUserId(params.userId);
